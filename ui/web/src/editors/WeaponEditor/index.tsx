...
/** Build weapon sprite filename */
function buildWeaponSpriteName(
  weaponId: string,
  animationState: AnimationState,
  direction: Direction,
  frame: number
): string {
  return `${weaponId}_${animationState}_${direction}_${frame}.png`;
}

/** Weapon sprite editor with animations and anchor point */
export function WeaponEditor() {
  ...
  const [anchorX, setAnchorX] = useState(16); // Center by default
  const [anchorY, setAnchorY] = useState(16);
  ...
  const [existingWeapons, setExistingWeapons] = useState<string[]>([]);
  ...
  // Load existing weapons on mount
  useEffect(() => {
    async function loadWeapons() {
      try {
        const storage = createStorage();
        const files = await storage.list('weapons');
        const ids = [
          ...new Set(
            files
  ...
  // Load weapon from storage
  const loadWeapon = useCallback(async (id: string) => {
    setIsLoading(true);
    setSaveError(null);
    try {
      const storage = createStorage();
      ...
      const def: WeaponDefinition = JSON.parse(defJson);
      setWeaponId(def.id);
      setWeaponName(def.name);
      setWeaponType(def.weaponType as WeaponType);
      setFrameCount(def.frames);
      setFrameDuration(def.frameDuration);
      setAnchorX(def.anchorX);
      setAnchorY(def.anchorY);
      ...
      for (const as of ANIMATION_STATES) {
        for (const dir of DIRECTIONS) {
          for (let f = 0; f < def.frames; f++) {
            const filename = buildWeaponSpriteName(id, as, dir, f);
            try {
              const url = storage.getReadUrl(`weapons/${id}/${filename}`);
      ...
      setSaveError(`Failed to load weapon: ${e}`);
    } finally {
      setIsLoading(false);
    }
  }, [getSpriteKey, loadSprite]);

  // Save weapon to storage
  const saveWeapon = useCallback(async () => {
    if (!weaponId.trim()) {
      setSaveError('Weapon ID is required');
      return;
    }
    ...
    try {
      ...
      await storage.writeText(
        `weapons/${weaponId}/definition.json`,
      ...
      for (const [key, data] of spritesRef.current.entries()) {
        ...
        const filename = buildWeaponSpriteName(
          weaponId,
        ...
        await storage.writeBytes(
          `weapons/${weaponId}/${filename}`,
          await blob.arrayBuffer(),
          'image/png'
        );
      }

      // Update existing weapons list
      if (!existingWeapons.includes(weaponId)) {
        setExistingWeapons([...existingWeapons, weaponId]);
      }

      console.log(`Saved weapon ${weaponId} with ${spritesRef.current.size} sprites`);
    } catch (e) {
      setSaveError(`Failed to save: ${e}`);
    } finally {
      setIsSaving(false);
    }
  }, [weaponId, weaponName, weaponType, frameCount, frameDuration, anchorX, anchorY, saveCurrentSprite, existingWeapons]);
  ...
  }, [currentKey, frame, frameCount, animationState, direction, getSpriteKey, saveCurrentSprite]);
  ...
  return (
    <div style={{ display: 'flex', height: '100%', minHeight: 'calc(100vh - 60px)' }}>
      ...
      >
        ...
        {existingWeapons.map((id) => (
          <div
            key={id}
            onClick={() => loadWeapon(id)}
            style={{
              padding: '0.5rem',
              background: id === weaponId ? '#4ecca3' : '#0f0f23',
              color: id === weaponId ? '#1a1a2e' : '#ccc',
              borderRadius: '4px',
              cursor: 'pointer',
        ...
        >
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <input
              type="text"
              value={weaponId}
              onChange={(e) => setWeaponId(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))}
              placeholder="weapon_id"
              style={{
                background: '#0f0f23',
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <span style={{ color: '#888', fontSize: '0.875rem' }}>Type:</span>
            <select
              value={weaponType}
          ...
          <button
            ...
            style={{
              ...
              border: 'none',
              borderRadius: '4px',
              cursor: isSaving || !weaponId ? 'not-allowed' : 'pointer',
              fontWeight: 'bold',
            }}
          >
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                    <span style={{ color: '#888', fontSize: '0.75rem' }}>X:</span>
                    <input
                      type="number"
                      value={anchorX}
                      onChange={(e) => setAnchorX(Math.max(0, Math.min(SPRITE_WIDTH - 1, parseInt(e.target.value, 10) || 0)))}
                      min={0}
                      max={SPRITE_WIDTH - 1}
                      style={{
                        background: '#0f0f23',
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                    <span style={{ color: '#888', fontSize: '0.75rem' }}>Y:</span>
                    <input
                      type="number"
                      value={anchorY}
                      onChange={(e) => setAnchorY(Math.max(0, Math.min(SPRITE_HEIGHT - 1, parseInt(e.target.value, 10) || 0)))}
                      min={0}
                      max={SPRITE_HEIGHT - 1}
                      style={{
                        background: '#0f0f23',
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', cursor: 'pointer' }}>
                    ...
                    />
                    <span style={{ color: '#888', fontSize: '0.75rem' }}>Show</span>
                  </label>
                </div>
              </div>
            </div>
