...
export function CharacterSheetEditor() {
  // Character list
  const [characters, setCharacters] = useState<ManifestCharacter[]>([]);
  const [selectedCharId, setSelectedCharId] = useState<string | null>(null);

  // Current character schema
  const [schema, setSchema] = useState<CharacterAtlasSchema | null>(null);
  const [atlasUrl, setAtlasUrl] = useState<string | null>(null);

  // Selected cell for detailed editing
  const [selectedCell, setSelectedCell] = useState<{ row: number; col: number } | null>(null);
  const [hoveredCell, setHoveredCell] = useState<CharacterCell | null>(null);
  ...
  const selectCharacter = useCallback(async (charId: string) => {
    ...
    const charSchema = buildCharacterSchema(
      char.id,
      char.name,
      char.animations,
      char.spriteSize
    );
    setSchema(charSchema);

    // Set atlas URL (use 'assets' basePath)
    const storage = createStorage({ basePath: 'assets' });
    const url = storage.getReadUrl(char.atlas);
    setAtlasUrl(url);
  }, [characters]);

  // Handle cell selection
  const handleCellSelect = useCallback((cell: CharacterCell | null) => {
    if (!cell) return;
  ...
  const handleCellHover = useCallback((cell: unknown) => {
    setHoveredCell(cell as CharacterCell | null);
  }, []);
  ...
  const saveAtlas = useCallback(async () => {
    if (!selectedCharId || !sheetRef.current || !schema) return;

    setIsSaving(true);
    setError(null);

    try {
      // Use 'assets' basePath for writing to public/assets/
      const storage = createStorage({ basePath: 'assets' });
      ...
      const atlasPath = `characters/${selectedCharId}.png`;
  ...
  }, [selectedCharId, schema]);

  // Copy cell to adjacent cells
      ...
      >
        ...
        {characters.map(char => (
          <div
            ...
            style={{
              padding: '0.5rem',
              background: char.id === selectedCharId ? '#4ecca3' : '#0f0f23',
              color: char.id === selectedCharId ? '#1a1a2e' : '#ccc',
              borderRadius: '4px',
              cursor: 'pointer',
              marginBottom: '0.25rem',
            }}
          >
            <div style={{ fontWeight: 'bold', fontSize: '0.875rem' }}>{char.name}</div>
            <div style={{ fontSize: '0.75rem', opacity: 0.7 }}>{char.id}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        >
          <span style={{ color: '#4ecca3', fontWeight: 'bold' }}>
            {selectedCharId ? `Editing: ${selectedCharId}` : 'Select a character'}
          </span>
          ...
          <button
            onClick={saveAtlas}
            disabled={isSaving || !selectedCharId || !hasUnsavedChanges}
            style={{
              padding: '0.5rem 1rem',
            ...
            {schema && atlasUrl ? (
              <>
                ...
                <SpriteSheetEditor
                  ...
                  onCellSelect={(cell) => handleCellSelect(cell as CharacterCell)}
              ...
              <div style={{ color: '#666', textAlign: 'center', paddingTop: '4rem' }}>
                Select a character from the list to edit its sprite sheet
              </div>
            )}
          </div>
